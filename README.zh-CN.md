...
- Windows 在运行 ASC 前将工作进程加入 Job Object；结束时一并清理子孙进程，不依赖父进程
  仍然存活。Job 设置失败则不启动分析。原生 CI 覆盖孤儿清理、输出超限及重复调用资源释放。
...
uv run python scripts/validate_corpus.py /APK路径/one.apk /APK路径/two.apk
